...
import { registerSupportDevice } from "@/lib/support-api";
...
export default function RootLayout() {
  ...
  useEffect(() => {
    ...
    const setupNotifications = async () => {
      ...
      if (!cancelled) await registerSupportDevice(token);
    };
    setupNotifications().catch(() => undefined);
    const responseListener = Notifications.addNotificationResponseReceivedListener((response) => {
      const url = response.notification.request.content.data?.url;
      if (typeof url === "string" && url.startsWith("qingliao://")) {
        router.push(url.replace("qingliao://", "/") as never);
      }
    });
    return () => {
      cancelled = true;
