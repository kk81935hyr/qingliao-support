...
var supportConversations = mysqlTable("support_conversations", {
  id: int("id").autoincrement().primaryKey(),
...
var supportMessages = mysqlTable("support_messages", {
  id: int("id").autoincrement().primaryKey(),
...
async function createSupportConversation(visitorToken, visitorLabel) {
  const db = await getDb();
...
async function getSupportConversationByToken(visitorToken) {
  const db = await getDb();
  if (!db) throw new Error("\u5BA2\u670D\u670D\u52A1\u6682\u65F6\u4E0D\u53EF\u7528");
...
async function listSupportMessages(conversationId) {
  const db = await getDb();
  if (!db) throw new Error("\u5BA2\u670D\u670D\u52A1\u6682\u65F6\u4E0D\u53EF\u7528");
...
async function createSupportMessage(input) {
  ...
  await db.update(supportConversations).set({ lastMessageAt: /* @__PURE__ */ new Date(), status: "open" }).where(eq(supportConversations.id, input.conversationId));
}
...
var appRouter = router({
  ...
  support: router({
    start: publicProcedure.mutation(async () => {
      ...
      const conversation = await createSupportConversation(visitorToken, visitorLabel);
      return conversation;
    }),
    inbox: publicProcedure.query(() => listSupportConversations()),
    messages: publicProcedure.input(z2.object({ visitorToken: z2.string().length(32) })).query(async ({ input }) => {
      const conversation = await getSupportConversationByToken(input.visitorToken);
      if (!conversation) return { conversation: null, messages: [] };
      const messages = await listSupportMessages(conversation.id);
      return { conversation, messages };
    }),
    send: publicProcedure.input(z2.object({
      visitorToken: z2.string().length(32),
      sender: z2.enum(["visitor", "agent"]),
      body: z2.string().trim().max(1e3).optional(),
      imageBase64: z2.string().max(22e5).optional(),
      imageMime: z2.enum(["image/jpeg", "image/png", "image/webp"]).optional()
    }).refine((input) => Boolean(input.body || input.imageBase64), "\u6D88\u606F\u5185\u5BB9\u4E0D\u80FD\u4E3A\u7A7A")).mutation(async ({ input }) => {
      const conversation = await getSupportConversationByToken(input.visitorToken);
      ...
      if (input.imageBase64 && input.imageMime) {
        ...
        if (buffer.byteLength > 16e5) throw new Error("\u56FE\u7247\u4E0D\u80FD\u8D85\u8FC7 1.5MB");
