...
export default function HomeScreen() {
  ...
  const inboxQuery = trpc.support.inbox.useQuery(undefined, { refetchInterval: 2500, refetchOnWindowFocus: true });
  ...
  return (
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
      <View className="pt-3 pb-4 flex-row items-center justify-between">
        ...
        <Pressable onPress={() => inboxQuery.refetch()} style={({ pressed }) => [{ backgroundColor: colors.primary }, pressed && { opacity: 0.78, transform: [{ scale: 0.96 }] }]} className="h-12 w-12 rounded-2xl items-center justify-center">
