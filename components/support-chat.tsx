...
export function SupportChat({ visitorToken, sender, title, subtitle, onBack, onSessionInvalid }: SupportChatProps) {
  ...
  const sendText = async () => {
    ...
    setSendError(null);
    try {
      await sendSupportMessage({ visitorToken, sender, body });
      setDraft("");
      await refreshMessages();
    } catch (error) {
      if (error instanceof Error && error.message.includes("不存在")) onSessionInvalid?.();
      else setSendError("发送失败，请重试");
    } finally {
      setIsSending(false);
    }
  };
  ...
  const sendImage = async () => {
    ...
    setSendError(null);
    try {
      await sendSupportMessage({ visitorToken, sender, imageBase64: asset.base64, imageMime });
      await refreshMessages();
    } catch (error) {
      if (error instanceof Error && error.message.includes("不存在")) onSessionInvalid?.();
      else setSendError("图片发送失败，请重试");
    } finally {
      setIsSending(false);
    }
  };
